...
class MSTNode(ABC):
	...
	def put(self, key: KTYPE, val: VTYPE) -> Self:
		if self.subtrees == (None,): # special case for empty tree
			return self._put_here(key, val)

		return self._put_recursive(key, val, self.key_height(key), self.height())

	
	def _put_recursive(self, key: KTYPE, val: VTYPE, key_height: int, tree_height: int) -> Self:
	...
	def _put_here(self, key: KTYPE, val: VTYPE) -> Self:
		cls = self.__class__ # maybe this could be a class method???
